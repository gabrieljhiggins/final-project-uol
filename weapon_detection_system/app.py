#!/usr/bin/env python3
"""
Multi-Camera Weapon Detection Dashboard - True Chronological Sync
Run with: python app.py --port 8080
"""
...
ANNOTATED_DIR = BASE_DIR.parent / "outputs" / "annotated"
TIMELINE_JSON = BASE_DIR.parent / "outputs" / "timeline.json"
...
@app.route("/api/cams")
def api_cams():
    # Force logical display order: Cam7 (earliest) → Cam5 → Cam1 (latest)
    all_cams = get_available_cams(ANNOTATED_DIR)
    preferred_order = ["Cam7", "Cam5", "Cam1"]
...
def download_archive():
    ...
    with zipfile.ZipFile(zpath, 'w') as zf:
        ...
        for ev in events[:30]:  # limit to first 30 events
            cam = ev.get("camera")
...
if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="Multi-Camera Chronological Dashboard")
    p.add_argument("--port", type=int, default=8080)
    p.add_argument("--output-dir", type=Path, default=None,
                   help="Override path to the outputs/ folder (must contain annotated/ and timeline.json)")
    args = p.parse_args()
